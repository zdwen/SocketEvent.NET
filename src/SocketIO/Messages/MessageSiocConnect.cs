...
namespace SocketIOClient.Messages
{
    ...
    public class MessageSiocConnect : MessageSioc
    {
        ...
        public override string Event { get { return "connect"; } }

        public MessageSiocConnect()
            : base()
        {
            MessageType = SocketIOMessageTypes.Connect;
        }

        public MessageSiocConnect(string endPoint)
            : this()
        {
            Endpoint = endPoint;
        }

        public static MessageSiocConnect Deserialize(string rawMessage)
        {
            MessageSiocConnect msg = new MessageSiocConnect();
            //  1:: [path] [query]
            //  1::/test?my=param
        ...
        public override string Encoded
        {
            get
            {
                return string.Format("1::{0}{1}", Endpoint, string.IsNullOrEmpty(Query) ? string.Empty : string.Format("?{0}", Query));
            }
        }
    }
}
